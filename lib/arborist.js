...
class Arborist extends EE {
  ...
  loadActual () {
    ...
    return realpath(this.root, this.rpcache, this.stcache)
      .then(() => this.loadFSNode({ path: this.root }))
      .then(node => {
  ...
  loadFSNode ({ path, parent }) {
    ...
    const p = realpath(path, this.rpcache, this.stcache).then(real =>
  ...
  loadFSChildren (node) {
    if (node.then)
      node.then(node => this.loadFSChildren(node))
