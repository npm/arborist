...
module.exports = cls => class Reifier extends Ideal(cls) {
  ...
  [_diffTrees] () {
    ...
    for (const path of this.diff.removed) {
  ...
  [_saveIdealTree] (options = {}) {
    ...
    //
    // XXX this needs to clone rather tha copy, so that we can leave failed
