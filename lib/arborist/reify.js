  ...
  async [_copyIdealToActual] () {
    ...
    if (this.diff && this.diff.filterSet.size) {
      ...
      for (const [loc, ideal] of this.idealTree.inventory.entries()) {
        ...
        if (this.diff.filterSet.has(ideal))
          continue
        ...
        if (!actual)
          ...
          // we may have already migrated over the target, but moving
          // over a Link with a null target will delete it, so we have
          // to make sure we don't drop that.
          const { target: idealTarget } = ideal
          actual.root = this.idealTree
          if (idealTarget && actual.isLink)
            actual.target = idealTarget
        }
      }
