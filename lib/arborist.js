...
class Arborist extends EE {
  ...
  moveAsideShallowNodes () {
    const moves = this.reifyShallowMoves = {}
    ...
    return promiseAllRejectLate(movePromises)
      .catch(/* istanbul ignore next - XXX */ er => this.rollbackMoveAsideShallowNodes(er))
  }
/* istanbul ignore next - XXX */ 
  rollbackMoveAsideShallowNodes (er) {
    const moves = this.reifyShallowMoves
    const movePromises = Object.entries(moves)
      .map(([from, to]) => rename(to, from))
    return promiseAllRejectLate(movePromises)
  ...
  createSparseTree () {
    // XXX: if we call this fn again, look for the previous list
    // so that we can avoid making the same directory multiple times
    const dirs = this.sparseTreeDirs = this.diff.leaves
      .filter(diff => diff.action === 'ADD' || diff.action === 'CHANGE')
      .map(diff => diff.ideal.path)

    return promiseAllRejectLate(dirs.map(d => mkdirp(d)))
      .catch(/* istanbul ignore next - XXX */ er => this.rollbackCreateSparseTree(er))
  }
/* istanbul ignore next - XXX */ 
  rollbackCreateSparseTree (er) {
    // cut the roots of the sparse tree, not the leaves
    const moves = this.reifyShallowMoves
    promiseAllRejectLate(Object.entries(moves).map(([from, to]) => rimraf(from)))
      .catch(er => {})
      .then(() => this.rollbackMoveAsideShallowNodes(er))
  }
  ...
  moveBackRetiredUnchanged () {
    ...
    promiseAllRejectLate(this.diff.children.map(diff => {
      if (diff.action !== 'CHANGE' || !diff.unchanged.length)
        return

      const realFolder = diff.ideal.path
      const retireFolder = moves[realFolder]
      retiredUnchanged[retireFolder] = []
      return Promise.all(diff.unchanged.map(node => {
        ...
        return mkdirp(dir).then(() =>
          this.moveContents(node, fromPath))
      }))
    }))
      .catch(/* istanbul ignore next - XXX */ er => this.rollbackMoveBackRetiredUnchanged(er))
  }
  /* istanbul ignore next - XXX */ rollbackMoveBackRetiredUnchanged (er) {
    console.error('ROLL BACK MOVE BACK RETIRED UNCHANGED', er)
  ...
  removeRetiredAndDeletedNodes () {
    ...
    const rm = path => rimraf(path)
      .catch(/* istanbul ignore next - XXX */ er => failures.push([path, er]))
  ...
  reify (options = {}) {
    return this.loadActual()
      ...
      .then(() => this.diffTrees())
      .then(() => this.moveAsideShallowNodes())
      .then(() => this.createSparseTree())
