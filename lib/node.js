...
class Node {
  ...
  set root (root) {
    ...
    // delist from old root
    oldRoot.inventory.delete(this)
    if (oldRoot.meta)
      oldRoot.meta.delete(this.path)
  ...
  set parent (parent) {
    ...
    if (this.path && (pathChange || topChange || rootChange)) {
      const delist = node => {
        oldRoot.inventory.delete(node)
        if (oldRoot.meta)
          oldRoot.meta.delete(node.path)
        if ((pathChange || topChange) && oldTop.meta && oldTop !== oldRoot)
          oldTop.meta.delete(node.path)
        node.children.forEach(c => delist(c))
      }
      delist(this)
    }
    ...
    if (parent) {
      const updatePath = (ppath, node) => {
        node.path = resolve(ppath, 'node_modules', node.name)
        if (!node.isLink)
          node.realpath = node.path
        node.children.forEach(c => updatePath(node.path, c))
      }
      updatePath(parent.path, this)
    }
    ...
    if (this.root === newRoot) {
      ...
      // to manually update these things.
      // XXX: would be better to set a dirty flag or something and DRY it
      this.location = relative(newRoot.path, this.path).replace(/\\/g, '/')
      this.root.inventory.add(this)
      if (this.root.meta)
        this.root.meta.add(this)
    } else
      this.root = newRoot

    // if the top is not the root, and it has meta, then we're updating
    // nodes within a link target's folder.  update that metadata now
    if (newTop !== newRoot && newTop.meta) {
      const updateTopMeta = node => {
        newTop.meta.add(node)
        node.children.forEach(c => updateTopMeta(c))
      }
      updateTopMeta(this)
    }
