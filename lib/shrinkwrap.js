...
class Shrinkwrap {
  ...
  [_updateWaitingNode] (loc) {
    ...
    //
    // XXX should be node.top.path !== this.path
    // or else it WILL include non-top link targets
    //
    // XXX Also, it's not quite right that they didn't track it at all.
    // We need to figure out the "effective" path, nested under the
    // link node somewhere.  (Weird choice, but it is what it is.)
