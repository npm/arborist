...
class Arborist extends EE {
  ...
  buildIdealTree (options = {}) {
    ...
    if (typeof update.depth !== 'number')
      update.depth = Infinity

    this.buildIdealTreeUpdateSettings = update

    const mutateTree = options.add || options.rm ||
      update.all || update.names.length

    const freshTree = update.depth === Infinity && update.all
    ...
    return rpj(this.path + '/package.json')
      ...
      .then(() => {
        if (update.names.length && update.depth === Infinity)
      ...
      .then(() => {
        ...
        if (!this.idealTree.meta.loadedFromDisk || !mutateTree)
          return
      ...
      .then(() => {
        ...
        // but don't bother looking if we didn't load from a shrinkwrap or
        ...
        // we started from a shrinkwrap, and then added/removed something,
  ...
  idealTreeQueueNamedUpdates (names) {
    ...
    for (const node of this.idealTree.inventory.filter(updateNode)) {
      for (const edge of node.edgesIn) {
        // depth doesn't matter because we only do this when
        // updateDepth is Infinity.
        this.buildIdealDepsQueue.push({ node: edge.from, depth: 0 })
      }
    }
  }
  ...
  idealTreeAddRm (options) {
    ...
    for (const [type, specs] of Object.entries(add)) {
      if (type === 'bundleDependencies') {
        ...
        // Note: need to use the existing value if it's '', and
  ...
  buildIdealDeps (node) {
    this.buildIdealDepsQueue.push({ node, depth: 0 })

    return this.buildIdealDepsStep()
  }

  buildIdealDepsStep () {
    const step = this.buildIdealDepsQueue.shift()
    if (!step)
      return
    ...
    if (this.buildIdealDepsSeen.has(node) || node.root !== this.idealTree)
      return this.buildIdealDepsStep()
    ...
    const shouldUpdate = edge => updateAll || updateNames.includes(edge.name)

    const problems = [...node.edgesOut.values()]
      .filter(edge => !bundled.has(edge.name) &&
        (!edge.valid || shouldUpdate(edge)))

    // if we are within the update depth, and either updating all or
    // updating named nodes, then we _also_ need to add nodes to the
    // update queue that are _not_ problems, just so we visit them
    // and check.
    if (depth < updateDepth && (updateNames.length || updateAll)) {
      for (const edge of node.edgesOut.values()) {
        if (!bundled.has(edge.name) && edge.valid && !shouldUpdate(edge)) {
          this.buildIdealDepsQueue.push({
            node: edge.to,
            depth: depth + 1,
          })
        }
  ...
  placeIdealDep (dep, node, edge, depth = 0, peerEntryEdge = null) {
    ...
    for (const edge of dep.edgesIn) {
      if (!edge.valid)
        this.buildIdealDepsQueue.push({ node: edge.from, depth: depth + 1 })
    }
      ...
      for (const peerEdge of dep.edgesOut.values()) {
        if (peerEdge.peer && !peerEdge.valid) {
          ...
          const peerPlaced = this.placeIdealDep(
            peer, dep, peerEdge, depth, peerEntryEdge || edge)
  ...
  canPlaceDep (dep, target, edge, peerEntryEdge = null) {
    ...
    // of this is the edge we're already looking to resolve!
    if (target !== edge.from && target.edgesOut.has(dep.name)) {
    ...
    const current = target !== edge.from && target.resolve(dep.name)
    if (current) {
