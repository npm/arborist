...
class Shrinkwrap {
  static load (path, indent, swonly) {
    return new Shrinkwrap(path, indent).load(swonly)
  }
  ...
  [_metaFromLock] (location, name, lock) {
    ...
    if (spec.type === 'directory') {
      ...
      meta.link = true
      meta.resolved = relative(resolve(this.path, location), spec.fetchSpec)
