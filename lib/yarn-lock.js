...
class YarnLock {
  ...
  fromTree (tree) {
    ...
    for (const node of tree.inventory.values()) {
      const specs = [...node.edgesIn].map(e => `${node.name}@${e.spec}`)
      this.current = new YarnLockEntry(specs)
      ...
      if (node.resolved) {
        const rspec = npa.resolve(node.name, node.resolved, node.path)
        if (rspec.type === 'file' || rspec.type === 'directory')
          this.current.resolved = `file:${
            relative(node.root.path, rspec.fetchSpec).replace(/\\/g, '/')}`
        else
          this.current.resolved = rspec.saveSpec
      }
      if (node.integrity)
