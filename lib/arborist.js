...
class Arborist extends EE {
  ...
  buildIdealTree (options = {}) {
    ...
    return rpj(this.path + '/package.json')
      ...
      .then(() => {
        // if we started with an empty tree, then the dep flags are
        // already all set to true, and there can be nothing extraneous,
        // so there's nothing to prune, because we built it from scratch.
        // if we didn't add or remove anything, then also nothing to do.
  ...
  buildIdealDepsStep () {
    ...
    // but not listed as bundled deps.  When reifying, it's important to
    // remember to skip those and not overwrite what came from the bundle,
    // but we accept this slight discrepancy at this point, because it doesn't
    // cause any worse problems than a few unnecessary packument fetches.
