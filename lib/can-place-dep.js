...
class CanPlaceDep {
  ...
  constructor (options) {
    ...
    debug(() => {
      if (parent && parent.children.some(c => c.dep === this.dep))
    ...
    // console.error('CPD result', {
    // target: this.target.location,
    // dep: [this.dep.name, this.dep.version],
    // current: this.current && [this.current.location, this.current.version],
    // edge: [this.edge.from.location, this.edge.type, this.edge.name, this.edge.spec],
    // canPlace: this.canPlace,
    // canPlaceSelf: this.canPlaceSelf,
    // })
  ...
  checkCanPlace () {
    ...
    const cpd = current ? this.checkCanPlaceCurrent()
      : this.checkCanPlaceNoCurrent()
    // console.error('result', {
    // cpd,
    // self: this.canPlaceSelf,
    // target: this.target.location,
    // current: this.current && [this.current.location, this.current.version],
    // dep: [this.dep.name, this.dep.version],
    // edge: [this.edge.from.location, this.edge.type, this.edge.name, this.edge.spec],
    // entryEdge: this.peerEntryEdge &&
    ...
    for (const [entryEdge, currentPeers] of peerEntrySets(current)) {
      ...
      if (entryRep) {
        if (entryRep.canReplace(entryNode, dep.parent.children.keys())) {
          // console.error('>>> entryReplacement can replace entryNode', [entryRep.name, entryRep.version])
          continue
        }
        // console.error('>>> have entryReplacement, cannot replace trivially', [entryRep.name, entryRep.version], [...entryNode.edgesIn].map(e => [e.from.location, e.type, e.name, e.spec]))
      }

      let canClobber = !entryRep
      if (!entryRep) {
        // console.error('>>> no replacement for entryNode, check peer set')
        const peerReplacementWalk = new Set([entryNode])
        OUTER: for (const currentPeer of peerReplacementWalk) {
          // console.error('>>> peerReplacementWalk', [currentPeer.location, currentPeer.version])
          for (const edge of currentPeer.edgesOut.values()) {
            if (!edge.peer || !edge.valid)
              continue
            // console.error('>>>> peerRep edge', [edge.from.location, edge.type, edge.name, edge.spec])
            const rep = dep.parent.children.get(edge.name)
            if (!rep) {
              // console.error('>>>>> no replacement')
              if (edge.to)
                peerReplacementWalk.add(edge.to)
              continue
            }
            if (!rep.satisfies(edge)) {
              // console.error('>>>>> replacement does not satisfy', [rep.name, rep.version])
              canClobber = false
              break OUTER
            }
            // console.error('>>>>> replacement ok!', [rep.name, rep.version])
          }
        }
      }
      if (canClobber)
      ...
      for (const currentPeer of currentPeers) {
        // console.error('>> check canNest, currentPeer', [currentPeer.location, currentPeer.version], currentPeer === entryEdge.to)
        if (!canNestCurrent)
          break
  ...
  canPlacePeers (state) {
    // console.error('CAN PLACE PEERS', { dep: [this.dep.name, this.dep.version], path: this.peerPath.map(d => [d.name, d.version]) })
