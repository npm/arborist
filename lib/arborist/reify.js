...
const _bundleBinLinks = Symbol('bundleBinLinks')
...
// XXX move to Arborist.rebuild() method
...
module.exports = cls => class Reifier extends Ideal(cls) {
  ...
  async [_reifyPackages] () {
    ...
    await this[_runLifecycleScripts]()
  ...
  [_loadTrees] (options) {
    ...
    const bitOpt = { ...options, complete: this[_packageLockOnly] }
  ...
  [_loadBundlesAndUpdateTrees] (
    ...
    .then(nodes =>
      ...
      }).then(() => this[_bundleBinLinks](node)))))
    // move onto the next level of bundled items
    .then(() => this[_loadBundlesAndUpdateTrees](depth + 1, bundlesByDepth))
    .catch(er => this[_rollbackCreateSparseTree](er))
  }

  [_bundleBinLinks] (node) {
    // create all the bin links for bundle dependencies
    const promises = []
    dfwalk({
      tree: node,
      visit: node => promises.push(this[_binLinks](node)),
  ...
  // XXX move this section to Arborist.rebuild()
