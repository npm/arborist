...
module.exports = cls => class Reifier extends Ideal(cls) {
  ...
  [_binLinks] (node) {
    // maybe should check that the parent is the root, not just top?
    // dont' necessarily want link targets getting all ../bin about it.
    const top = node.isTop ||
      this[_global] && node.parent.isTop
  ...
  [_saveIdealTree] (options) {
    ...
    return Promise.all([
      !this[_global] && this.idealTree.meta.save(),
