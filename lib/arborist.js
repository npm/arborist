...
class Arborist extends EE {
  ...
  buildIdealTree (options = {}) {
    ...
    return rpj(this.path + '/package.json')
      ...
      .then(root => !update.all ? this.loadVirtual({ root })
      ...
      // If we have a list of package names to update, add any paths into
      // those named nodes to the buildIdealTree queue.
      .then(() => {
      ...
      .then(() => {
        // if we started with an empty tree, or built a fresh tree
        // for update.all, then the dep flags are already all set to true,
        // and there can be nothing extraneous, so there's nothing to prune,
        // because we built it from scratch.  if we didn't add or remove
        // anything, then also nothing to do.
        if (!this.idealTree.meta.loadedFromDisk ||
            !options.add && !options.rm)
          return
      ...
      .then(() => {
        // if we started from a blank slate, or added/removed something, then
        // the dep flags will be all set to true.
        if (!this.idealTree.meta.loadedFromDisk || options.add || options.rm)
          calcDepFlags(this.idealTree)
      })

      .then(() => {
        // at this point, any node marked as extraneous should be pruned.
        // but don't bother looking if we didn't load from a shrinkwrap and
        // then add/remove something.
        if (options.prune === false ||
            !this.idealTree.meta.loadedFromDisk ||
            !options.rm && !options.add)
          return
  ...
  buildIdealDepsStep () {
    ...
    const { names: updateNames } = this.buildIdealTreeUpdateSettings
    ...
    .then(placed => {
      ...
      // sort shallower deps up to the front of the queue, then alphabetical
  ...
  placeIdealDep (dep, node, edge, peerEntryEdge = null) {
    ...
    const start = edge.peer && !node.isRoot
      ? /* istanbul ignore next - This only matters once we're doing
           deep updates of linked packages */
        node.parent || node.fsParent || node
      : node
    ...
    dassert(canPlace !== placeDep_KEEP,
      'keeping a dep even though the edge had an error?? ' + edge.error)
    ...
    // just in case some exotic sitution escaped my contemplation, or the
    // code at the top of this method changes in the future.
