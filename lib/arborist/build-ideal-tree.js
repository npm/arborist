  ...
  [_queueVulnDependents] (options) {
    ...
    if (this[_force] && this.auditReport && this.auditReport.topVulns.size) {
      ...
      for (const [name, topVuln] of this.auditReport.topVulns.entries()) {
        const { packument, simpleRange, range: avoid, topNodes } = topVuln
        for (const node of topNodes) {
          if (node !== this.idealTree) {
            ...
            this.log.warn('audit', 'Manual fix required in linked project ' +
              `at ./${node.location} for ${name}@${simpleRange} vulnerability`)
            continue
          }

          try {
            const { spec } = this.idealTree.edgesOut.get(name)
            const opts = { avoid, avoidStrict: true }
            const mani = pickManifest(packument, spec, opts)
            options.add.push(`${mani.name}@${mani.version}`)
            const outside = mani._outsideDependencyRange
            const major = mani._isSemVerMajor
            const breakingMessage = major ? 'a SemVer major change'
              : outside ? 'outside your stated dependency range'
              : /* istanbul ignore next - shouldn't ever hit this, as it
                   would be avoided without having to be forced */
                'allowed by the stated dependency range'

            this.log.warn('audit', `Updating ${mani.name} to ${mani.version}` +
              `which is ${breakingMessage}`)
          } catch (er) {
            this.log.warn('audit', `No fix available for ${
              name}@${simpleRange}`)
            this.log.silly('audit', er)
          }
        }
      }
    }
