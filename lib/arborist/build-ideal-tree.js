...
module.exports = cls => class IdealTreeBuilder extends Tracker(Virtual(Actual(cls))) {
  ...
  [_queueVulnDependents] (options) {
    ...
    // if there's a fix, we use that.  otherwise, we remove it.
    // XXX: do not rm when no fix is available, just tell the user to
    ...
    if (this[_force] && this.auditReport && this.auditReport.topVulns.size) {
      ...
      for (const [name, topVuln] of this.auditReport.topVulns.entries()) {
        const {packument, range: avoid, topNodes} = topVuln
        const { spec } = this.virtualTree.edgesOut.get(name)
        // try to install a version of the target that is not vulnerable,
        // favoring a non-semver-major if possible.
        const mani = pickManifest(packument, spec, { avoid, avoidStrict: true })
          .catch(() => null)
        if (topNodes.has(this.virtualTree) && mani)
