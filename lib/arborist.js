...
class Arborist extends EE {
  ...
  loadVirtual () {
    return Shrinkwrap.load(this.root).then(s => {
      if (!s.loadedFromDisk)
      ...
      for (const [location, meta] of links.entries()) {
        const targetPath = resolve(this.root, location, meta.resolved)
        const targetLoc = relative(this.root, targetPath)
  ...
  loadVirtualLink (location, targetLoc, target) {
    ...
    const link = new Link({
      path,
      realpath: resolve(this.root, targetLoc),
      target,
  ...
  loadFSNode ({ path, parent, real, root }) {
    ...
    // basically, if we have a parent, load with swonly=true
    const p = new Shrinkwrap(real).load(!!parent).then(meta => {
      // TODO --no-shrinkwrap support
      if (path !== this.root && !meta.loadedFromDisk)
        meta = null
