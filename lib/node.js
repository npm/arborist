...
class Node {
  constructor (options) {
    ...
    const parentDir = basename(dirname(physical))
    if (parentDir.charAt(0) === '@')
      this.name = `${parentDir}/${basename(physical)}`
    else
      this.name = basename(physical)
    ...
    // physical parent dir
    // set after children, so the setter doesn't get upset
    this.parent = parent
  ...
  [loadDepinfo] (depNodes) {
    ...
    // ordering is relevant here!
    // we load dependencies and peerDependencies first,
    // so that the optional and dev flags get set properly.
