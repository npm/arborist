...
class Vuln {
  ...
  toJSON () {
    const edgesIn = Array.from(this.nodes.values()).flatMap(a => Array.from(a.edgesIn.values())).map(a => a.toJSON())
    const types = new Set(edgesIn.map(a => a.type).sort())
    const isDirect = [...this.nodes.values()].some(n => [...n.edgesIn].some(e => e.from.isProjectRoot()))
    // Feels like there's a method for checking this and I just didn't find it
