...
module.exports = cls => class ActualLoader extends cls {
  ...
  async loadActual (options = {}) {
    ...
    if (global) {
      ...
      this.actualTree = await this[newNodeOrLink]({
        path: this.path,
        realpath: real,
        pkg: {},
        global,
      })
      return this[_loadActualActually]({root})
    }

    // not in global mode, hidden lockfile is allowed, load root pkg too
    this.actualTree = await this[_loadFSNode]({
      path: this.path,
    ...
    const meta = await Shrinkwrap.load({
      path: this.actualTree.realpath,
      hiddenLockfile: true,
    })
    if (meta.loadedFromDisk) {
      this.actualTree.meta = meta
      meta.add(this.actualTree)
  ...
  async [_loadActualActually] ({ root }) {
