...
class Node {
  ...
  set root (root) {
    ...
    if (oldRoot) {
      ...
      if (oldRoot.meta)
        oldRoot.meta.delete(this.location)
    }
    if (root.path)
    ...
    if (root.meta) {
      if (this.resolved === undefined && this.integrity === undefined) {
        const {resolved, integrity} = root.meta.get(this.location)
  ...
  set parent (parent) {
    ...
    if (oldParent) {
      ...
      // only delete from the old parent's top meta if:
      // - not the root (since that's handled elsewhere)
      // - not the same as the new parent's top (no need)
      const opt = oldParent.top
      if (parent && opt !== oldParent.root && opt !== parent.top && opt.meta)
        opt.meta.delete(this.location)
    }
  ...
  [_updateLocation] () {
    if (this.parent) {
      if (this.root.meta)
        this.root.meta.delete(this.location)
      if (this.top !== this.root && this.top.meta)
        this.top.meta.delete(this.location)
